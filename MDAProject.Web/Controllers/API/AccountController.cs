...
namespace MDAProject.Web.Controllers.API
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> PostUser([FromBody] UserRequest request)
        {
            ...
            if (request.RoleId == 1)
            {
                await _userHelper.AddUserToRoleAsync(userNew, "Manager");
                _dataContext.Managers.Add(new Manager { User = userNew });
            }
            else if(request.RoleId == 1)
            {
                await _userHelper.AddUserToRoleAsync(userNew, "Assistant");
                _dataContext.Assistants.Add(new Assistant { User = userNew });
            }else
            {
                await _userHelper.AddUserToRoleAsync(userNew, "WareHouseManager");
                _dataContext.WareHouseManagers.Add(new WareHouseManager { User = userNew });
            }
